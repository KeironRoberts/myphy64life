...
<div class="step active" style="border: 2px solid #4CAF50; padding: 20px; border-radius: 10px;">
  <h3>Step 1: Match Given Values</h3>
...
`,
  ...
  STEP3: (problem) => `
    <div class="step active" style="border: 2px solid #4CAF50; padding: 20px; border-radius: 10px;">
      <h3>Step 3: Calculate Final Answer</h3>
      <div style="font-size: 18px; margin: 20px 0;">${problem.unknown?.[0]?.formula ?? 'Loading...'} = ?</div>
      <div style="margin: 20px 0;">
        <label for="finalAnswer" style="display: block; margin-bottom: 10px; font-weight: bold;">Enter answer (meters):</label>
        <input type="number" id="finalAnswer" step="0.1" 
               value="${problem.unknown?.[0]?.answer ?? ''}" 
...
`,
  ...
  CALCULATOR: (given1, given2) => {
    ...
    return `
      <div style="position: absolute; bottom: 70px; left: 25px; right: 25px; top: 70px; background: rgba(255,255,255,0.98); border: 3px solid #2196F3; border-radius: 15px; padding: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.4); font-family: 'Courier New', monospace; z-index: 1000; overflow: auto;">
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; max-width: 300px; margin: 0 auto;">
          ...
          ${buttons.map(btn => `<button class="calc-btn" data-value="${btn === '⌫' ? 'BACKSPACE' : btn}" style="padding: 20px; font-size: 18px; border: 1px solid #ddd; border-radius: 10px; background: #e9ecef; cursor: pointer;">${btn}</button>`).join('')}
        </div>
