...
`
  ...
  <div class="variable-zones-wrapper">
    <div class="variables" style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center; margin-top: 20px;">
      ${[...problem.givens, ...problem.unknowns]
        .map(
          (g) => `
        <div class="drop-zone drop-small" tabindex="0" role="button" aria-dropeffect="none" data-var="${
          g.target
        }">
  ...
  <div class="variable-zones-wrapper">
    <div class="variables" style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center; margin-top: 20px;">
      ${problem.unknowns
        .map(
          (unknown) => `
        <div class="drop-zone drop-large" tabindex="0" role="button" aria-dropeffect="none" data-target="${
          unknown.target
        }">
...
`,
  ...
  <div class="step active step-panel">
    ...
    <div class="info-box">
      ...
      <div class="chip-list">
        ${solvedVariables
          .map((g) => `<span class="chip">${g.label} = ${g.target.toUpperCase()}</span>`)
          .join('')}
      </div>
    </div>
...
`,
  ...
  CALC_STEP: (problem, currentUnknown, stepIndex, solvedVariables = []) => {
    ...
    const givensHtml = [...problem.givens, ...solvedVariables]
      .map((g) => `<div class="given-chip">${g.label || g.value}: ${g.value}</div>`)
      .join('');
