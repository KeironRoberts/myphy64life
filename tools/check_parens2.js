...
for (let i = 0; i < s.length; i++) {
  ...
  if (c === '(') open++;
  if (c === ')') open--;
  if (open < 0 && firstNegative === null) firstNegative = { line, i };
}
console.log('net parentheses count (ignoring strings/comments):', open);
if (open !== 0) {
  console.log('Unbalanced parentheses detected; first negative at', firstNegative);
  // print +/- context lines
  const lines = s.split('\n');
  const L = Math.max(0, firstNegative.line - 3),
    R = Math.min(lines.length - 1, firstNegative.line + 3);
  for (let l = L; l <= R; l++) console.log(l + 1 + ': ' + lines[l]);
